...
const QuoteConversionCard = ({ data }) => {
    ...
    if (!data) {
        return (
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl border border-white/50 p-6 h-full flex items-center justify-center"
                ...
                <p className="text-slate-400">{t('analytics.noQuoteData')}</p>
            </div>
        );
    }
    ...
    return (
        <div className="bg-white/70 backdrop-blur-sm rounded-2xl border border-white/50 p-4 sm:p-5 lg:p-6 h-full flex flex-col overflow-hidden"
            style={{ boxShadow: '0 4px 24px -4px rgba(0, 0, 0, 0.06)' }}>
            <h3 className="text-sm sm:text-base font-bold text-slate-700 mb-3 sm:mb-4">{t('analytics.quoteConversion')}</h3>
            ...
            <div className="flex-1 flex flex-col justify-center space-y-3 sm:space-y-4">
                ...
                <div className="text-center p-4 sm:p-5 bg-gradient-to-br from-indigo-50/80 to-violet-50/80 rounded-xl border border-indigo-100/50">
                    <p className="text-[10px] uppercase tracking-wider text-slate-400 font-semibold mb-1">{t('analytics.conversionRate')}</p>
                    ...
                    <p className="text-[10px] sm:text-xs text-slate-400 mt-1">{t('analytics.quoteToInvoice')}</p>
                </div>

                {/* Breakdown Stats */}
                <div className="grid grid-cols-2 gap-2 sm:gap-3">
                    <div className="text-center p-2.5 sm:p-3 bg-slate-50/80 rounded-xl border border-slate-100/50">
                        <p className="text-[9px] sm:text-[10px] uppercase tracking-wider text-slate-400 font-semibold mb-1">{t('analytics.quotesCreated')}</p>
                        <p className="text-lg sm:text-xl font-bold text-slate-700">{quotes_created}</p>
                    </div>
                    <div className="text-center p-2.5 sm:p-3 bg-slate-50/80 rounded-xl border border-slate-100/50">
                        <p className="text-[9px] sm:text-[10px] uppercase tracking-wider text-slate-400 font-semibold mb-1">{t('analytics.invoices')}</p>
