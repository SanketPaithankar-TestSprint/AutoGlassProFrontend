...
export default function AnalyticsDatePicker({ onChange }) {
    ...
    const PRESETS = React.useMemo(() => [
        ...
        { key: 'yesterday', label: t('analytics.yesterday') },
        { key: 'this_week', label: t('analytics.thisWeek') },
        { key: 'last_7', label: t('analytics.last7Days') },
        { key: 'last_week', label: t('analytics.lastWeek') },
        { key: 'last_14', label: t('analytics.last14Days') },
        { key: 'this_month', label: t('analytics.thisMonth') },
        { key: 'last_30', label: t('analytics.last30Days') },
        { key: 'last_month', label: t('analytics.lastMonth') },
        { key: 'all_time', label: t('analytics.allTime') },
    ...
    const handlePreset = (key) => {
        ...
        const presetLabel = PRESETS.find(p => p.key === key)?.label ?? t('analytics.allTime');
    ...
    const currentLabel = React.useMemo(() => {
        if (activePreset !== 'custom') return PRESETS.find(p => p.key === activePreset)?.label ?? t('analytics.allTime');
        ...
        return t('analytics.customRange');
    }, [activePreset, customStart, customEnd, PRESETS, t]);

    const selectionHint = !customStart
        ? t('analytics.clickStartDate')
        : !customEnd ? t('analytics.clickEndDate')
                ...
                <>
                    ...
                    <div className="hidden sm:flex absolute right-0 mt-2 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden"
                        ...
                        <div className="w-56 border-r border-slate-100 py-3 flex-shrink-0 bg-slate-50">
                            <p className="px-4 pb-2 text-[11px] font-bold text-slate-400 uppercase tracking-widest">{t('analytics.quickSelect')}</p>
                            {PRESETS.map(p => (
                        ...
                        <div className="flex-1 p-5">
                            {activePreset === 'custom' ? (
                                <>
                                    ...
                                    <div className="flex items-center gap-3 mb-5">
                                        <div className="flex-1 cursor-pointer" onClick={() => { setStart(null); setEnd(null); }} title="Click to reset">
                                            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-1">{t('analytics.startDate')}</p>
                                            <div className={`px-3 py-2 border rounded-lg text-sm ${customStart ? 'border-blue-400 text-slate-800 bg-blue-50' : 'border-dashed border-slate-300 text-slate-400 bg-slate-50'}`}>
                                                {customStart ? customStart.format('ddd, DD MMM YYYY') : t('analytics.clickStartDate')}
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex-1">
                                            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-1">{t('analytics.endDate')}</p>
                                            <div className={`px-3 py-2 border rounded-lg text-sm ${customEnd ? 'border-blue-400 text-slate-800 bg-blue-50' : 'border-dashed border-slate-300 text-slate-400 bg-slate-50'}`}>
                                                {customEnd ? customEnd.format('ddd, DD MMM YYYY') : t('analytics.clickEndDate')}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="flex flex-col items-center justify-center h-full text-slate-400 text-sm gap-3 py-10">
                                    ...
                                    <p className="text-base text-slate-500">{t('analytics.customRange')}</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="sm:hidden fixed inset-x-3 top-16 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden max-h-[80vh] overflow-y-auto"
                        ...
                        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
                            <span className="font-bold text-slate-800">{t('analytics.selectPeriod')}</span>
                            <button type="button" onClick={() => setOpen(false)}
                            ...
                            <div className="px-4 pb-4">
                                <div className="flex items-center gap-2 mb-4">
                                    <div className="flex-1 px-3 py-2 border rounded-lg text-sm text-center cursor-pointer"
                                        ...
                                        {customStart ? customStart.format('DD MMM YYYY') : t('analytics.startDate')}
                                    </div>
                                    ...
                                    <div className="flex-1 px-3 py-2 border rounded-lg text-sm text-center"
                                        ...
                                        {customEnd ? customEnd.format('DD MMM YYYY') : t('analytics.endDate')}
                                    </div>
                                </div>
