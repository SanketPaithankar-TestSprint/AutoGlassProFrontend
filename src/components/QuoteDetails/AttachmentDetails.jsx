...
const AttachmentDetails = ({
    attachments = [],
    setAttachments,
    documentDetails // { documentNumber, customerId, userId }
}) => {
    ...
    const handleUpload = async () => {
        if (!documentDetails) {
            message.error("No active service document found.");
            return;
        }
        ...
        try {
            ...
            await uploadAttachments(files, descriptions, documentDetails);
            ...
            {/* Actions for Upload Workflow */}
            {
                documentDetails && (
                    <div className="mt-8 flex justify-end gap-3 pt-4 border-t border-slate-200">
                        ...
                        >
                            {attachments.length > 0 ? "Upload & Finish" : "Finish"}
                        </Button>
                    </div>
                )
            }
